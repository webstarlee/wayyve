        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder viewHolder, int position)
        {
            ...
            { 
                if (viewHolder is PlacesAdapterViewHolder holder)
                {
                    ...
                    if (item != null)
                    {
                        var drawable = TextDrawable.TextDrawable.TextDrawbleBuilder.BeginConfig().FontSize(35).EndConfig().BuildRound(item.Name.Substring(0, 1), Color.ParseColor(AppSettings.MainColor));
